...
class Res2Net(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample=downsample, stype="stage", baseWidth=self.baseWidth, scale=self.scale)
        )
...
def res2net50_v1b_26w_4s(pretrained=False, **kwargs):
    ...
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=4, **kwargs)
    if pretrained:
        local_ckpt = os.environ.get("CMSA_RES2NET50_PATH")
        if local_ckpt and os.path.isfile(local_ckpt):
            model_state = torch.load(local_ckpt, map_location="cpu")
