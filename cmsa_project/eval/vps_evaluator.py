...
def evaluator(gt_pth_lst, pred_pth_lst, metrics):
    ...
    for metric_module_name in metric_module_list:
        metric_module[metric_module_name] = getattr(__import__("metrics", fromlist=[metric_module_name]), metric_module_name)(
        ...
        with open(filename, "w+") as file_to_write:
            ...
            for _model_name in opt.model_lst[0]:
                ...
                for case in case_list:
                    ...
                    try:
                        case_gt_name_list.sort(key=lambda name: (int(name.split("/")[-2]), int(name.split("/")[-1].rstrip(".png"))))
                    except:
                        case_gt_name_list.sort(
                            key=lambda name: (
                                int(name.split("/")[-2].split("case")[1].split("_")[0]),
                                0 if not len(name.split("/")[-2].split("_")) > 1 else int(name.split("/")[-2].split("_")[1]),
                    ...
                    result = evaluator(gt_pth_lst=case_gt_name_list, pred_pth_lst=case_pred_name_list, metrics=opt.metric_list)
            ...
            # Save csv.
            csv_path = os.path.join(txt_save_path, "{}_eval.csv".format(_data_name.replace("/", "-")))
            with open(csv_path, "w+") as f_csv:
                # Write header.
                f_csv.write(",".join(tb.field_names) + "\n")
                # Write each row.
                for row in tb._rows:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--txt_name", type=str, help="logging root", default="Benchmark-CMSANet-pvtv2b2-epoch005-update")
    parser.add_argument("--check_integrity", type=bool, help="whether to check the file integrity", default=True)
    opt = parser.parse_args()

    # Provide defaults for data_lst and model_lst.
    if opt.data_lst is None:
        opt.data_lst = [["TestEasyDataset/Seen", "TestEasyDataset/Unseen", "TestHardDataset/Seen", "TestHardDataset/Unseen"]]

    if opt.model_lst is None:
        opt.model_lst = [["CMSANet_pvtv2b2_epoch005_update"]]

    txt_save_path = os.path.join(PROJECT_ROOT, "eval-result", opt.txt_name)
    ...
    if opt.check_integrity:
        print(">>> Checking integrity of prediction folders...")
        for _data_name in opt.data_lst[0]:
            for _model_name in opt.model_lst[0]:
                gt_pth = os.path.join(opt.gt_root, _data_name, "GT")
                pred_pth = os.path.join(opt.pred_root, _model_name, _data_name)

                if not os.path.isdir(gt_pth):
                    print(f"[WARN] GT path not found: {gt_pth}")
                    continue
                if not os.path.isdir(pred_pth):
                    print(f"[WARN] Pred path not found: {pred_pth}")
                    continue

                gt_items = sorted(os.listdir(gt_pth))
                pred_items = sorted(os.listdir(pred_pth))

                if gt_items != pred_items:
                    print("\n" + "=" * 60)
                    print(f"[MISMATCH] Dataset: {_data_name} | Model: {_model_name}")
                    print(f"GT   dir: {gt_pth}")
                    print(f"PRED dir: {pred_pth}")
                    print(f"GT   count : {len(gt_items)}")
                    print(f"PRED count: {len(pred_items)}")

                    # Calculate set differences.
                    missing_in_pred = sorted(set(gt_items) - set(pred_items))
                    extra_in_pred = sorted(set(pred_items) - set(gt_items))

                    if missing_in_pred:
                        print(f"  -> In GT but NOT in Pred (first 10):")
                        for name in missing_in_pred[:10]:
                            print(f"     [Missing] {name}")
                        if len(missing_in_pred) > 10:
                            print(f"     ... and {len(missing_in_pred) - 10} more")

                    if extra_in_pred:
                        print(f"  -> In Pred but NOT in GT (first 10):")
                        for name in extra_in_pred[:10]:
                            print(f"     [Extra]   {name}")
                        if len(extra_in_pred) > 10:
                            print(f"     ... and {len(extra_in_pred) - 10} more")

                    print("=" * 60 + "\n")
                else:
                    print(f"[OK] Dataset: {_data_name} | Model: {_model_name} | {len(gt_items)} folders match.")
    else:
        print(">>> Skip check the integrity of each candidates ...")
